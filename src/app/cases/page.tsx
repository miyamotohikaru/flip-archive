...
export default function IndexPage() {
  ...
  const [filters, setFilters] = useState<Partial<Record<AxisKey, string>>>(
    (r?.filters as Partial<Record<AxisKey, string>>) ?? {},
  );
  const [opFilter, setOpFilter] = useState<FlipOp | null>(
    (r?.opFilter as FlipOp | null) ?? null,
  );
  const [open, setOpen] = useState(r?.panelOpen ?? false);
  const [hovered, setHovered] = useState<Case | null>(null);
  const [mode, setMode] = useState<"OVERVIEW" | "LIST">(r?.mode ?? "OVERVIEW");
